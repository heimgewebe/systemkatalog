...
render "$REPO_ROOT/ops/systemd/heimgewebe-systemkatalog.service.tmpl" "$TMP/unit"
...
[[ "$(systemctl --user is-enabled heimgewebe-systemkatalog.service)" == enabled ]] || die "Unit nicht aktiviert"
...
assert data["kind"] == "heimgewebe_system_catalog"
assert len(data["systems"]) == 19
assert len(data["relations"]) == 24
with urllib.request.urlopen(base + "/", timeout=3) as response:
    assert response.status == 200 and b"Heimgewebe-Systemkatalog" in response.read()
try:
    socket.create_connection(("127.0.0.1", 4100), timeout=0.5)
except OSError:
    pass
else:
    raise SystemExit("STOP: alter Cabinet-Daemon-Port ist weiterhin offen")
PY

echo "TARGET-PROOF: HEIMGEWEBE SYSTEM CATALOG RUNTIME MATCHES REPOSITORY"
