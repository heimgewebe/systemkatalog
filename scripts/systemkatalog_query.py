...
def _data(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "resilienceBySystem": {item["system"]: item for item in resilience["systems"]},
        "resilienceByRelation": {
            (item["relation"]["from"], item["relation"]["to"], item["relation"]["type"]): item
            for item in resilience["relations"]
        },
        "failureDomains": {item["id"]: item for item in resilience["failureDomains"]},
        "recoveryModes": {item["id"]: item for item in resilience["recoveryModes"]},
    }
...
def _envelope(root: Path, command: str, result: Any) -> dict[str, Any]:
    return {
        ...
        "doesNotEstablish": [
            "runtime_health", "task_status", "merge_readiness", "execution_permission",
            "current_recovery_readiness", "automatic_failover_authority",
        ],
    }
...
def query(root: Path, command: str, value: str) -> dict[str, Any]:
    ...
    if command == "system":
        result = {"system": node, "sourceBinding": data["sourceBySystem"][node["id"]], "resilience": data["resilienceBySystem"][node["id"]]}
    elif command == "repository":
        if node["type"] != "repository":
            raise LookupError(f"not a repository system: {node['id']}")
        result = {"system": node, "repository": node["entrypoints"]["repository"], "sourceBinding": data["sourceBySystem"][node["id"]], "resilience": data["resilienceBySystem"][node["id"]]}
    elif command == "entrypoints":
        result = {"system": {"id": node["id"], "name": node["name"], "type": node["type"]}, "entrypoints": node["entrypoints"], "sourceBinding": data["sourceBySystem"][node["id"]], "resilience": data["resilienceBySystem"][node["id"]]}
        ...
        for edge in data["edges"]:
            ...
            relations.append({"relation": edge, "sourceBinding": data["sourceByRelation"][key], "resilience": data["resilienceByRelation"].get(key)})
        ...
        result = {"authority": authority, "ownerSystem": owner, "ownerSourceBinding": data["sourceBySystem"].get(owner["id"]) if owner else None, "ownerResilience": data["resilienceBySystem"].get(owner["id"]) if owner else None}
    elif command == "failure-domain":
        domain = data["failureDomains"].get(value)
        if domain is None:
            raise LookupError(f"unknown failure domain: {value}")
        result = {
            "failureDomain": domain,
            "systems": sorted(item["system"] for item in data["resilienceBySystem"].values() if value in item["failureDomains"]),
            "recoveryModes": sorted(item["id"] for item in data["recoveryModes"].values() if value in item["failureDomains"]),
        }
    elif command == "recovery-mode":
        mode = data["recoveryModes"].get(value)
        if mode is None:
            raise LookupError(f"unknown recovery mode: {value}")
        result = mode
    else:
...
def main() -> int:
    ...
    for name in ("system", "repository", "entrypoints", "relations", "truth-owner", "failure-domain", "recovery-mode"):
