...
DEFAULT_OUTPUT = ROOT / "rendered/system-catalog.md"
...
def render_text() -> str:
    ...
    for node in sorted(
        ...
        lines.append(
            f"| {_cell(node.get('label'))} | {_cell(node.get('kind'))} | {_cell(node.get('role'))} |"
        )
    ...
    for item in sorted(authorities, key=lambda value: str(value.get("domain", ""))):
        ...
        rendered_projections = ", ".join(str(value) for value in projections) if projections else "—"
        lines.append(
            f"| `{_cell(item.get('domain'))}` | `{_cell(item.get('owner'))}` | {_cell(rendered_projections)} |"
        )
    ...
    for edge in sorted(
        ...
        lines.append(
            "| "
            f"{_cell(source.get('label') or edge.get('from'))} | "
            f"`{_cell(edge.get('type'))}` | "
            f"{_cell(target.get('label') or edge.get('to'))} | "
            f"{_cell(edge.get('note'))} |"
        )
